<!-- vscode-markdown-toc -->
	* [Go在windows环境下build生成linux程序](#Gowindowsbuildlinux)
* [Go module](#Gomodule)
* [Go panic](#Gopanic)
* [new 和 make 的区别？](#newmake)
* [什么是协程（Goroutine）与线程区别](#Goroutine)
* [接口interface](#interface)
	* [2 个 interface 可以比较吗？ok](#interfaceok)
	* [go 的 interface 怎么实现的？](#gointerface)
	* [2 个 nil 可能不相等吗？ ok](#nilok)
* [map 的底层实现](#map)
* [go 里面的 map 是并发安全的吗？如何并发安全](#gomap)
* [链式调用](#)
* [Hook机制](#Hook)
* [orm](#orm)
* [gorm 加上 logger](#gormlogger)
* [gorm 里面如何批量插入某个字段](#gorm)
* [gorm 里面更新有几种方式？](#gorm-1)
* [前缀树](#-1)
	* [路由](#-1)
		* [路由分组](#-1)
		* [gin定义路由多种方式](#gin)
...
* [你项目有优雅的启停吗？](#-1)
	* [tips](#tips)
* [令牌桶](#-1)
* [goroutine 什么情况会发生内存泄漏？如何避免。](#goroutine)
* [如果若干个 goroutine，有一个 panic 会怎么做？](#goroutinepanic)
* [说说 atomic 底层怎么实现的.](#atomic.)
* [mutex 有几种模式？](#mutex)
...
* [WaitGroup](#WaitGroup)
* [调度流程](#-1)
* [Go 中 GMP 有哪些状态？](#GoGMP)
* [GMP 能不能去掉 P 层？会怎么样？](#GMPP)
* [如果有一个 G 一直占用资源怎么办？什么是 work stealing 算法？](#Gworkstealing)
* [channel底层结构](#channel)
* [向 channel 写数据的流程：](#channel-1)
* [向 channel 读数据的流程：](#channel-1)
* [对已经关闭的的 chan 进行读写，会怎么样？为什么？](#chan)
* [channel 死锁的场景](#channel-1)
* [channel 底层实现？是否线程安全。](#channel-1)
* [无缓冲的 channel 和有缓冲的 channel 的区别？](#channelchannel)
* [Go 语言中可以使用 channel 和 mutex 实现阻塞队列](#Gochannelmutex)
* [channel 为什么需要两个队列实现？](#channel-1)
* [go 通道和锁的使用场景？](#go)
...
* [写屏障](#-1)
* [混合写屏障](#-1)
* [知道golang的内存逃逸吗？什么情况下会发生内存逃逸？](#golang)
* [你是否主动关闭过 http 连接，为啥要这样做？ok](#httpok)
* [Go GC 有几个阶段](#GoGC)
* [golang 的内存管理的原理清楚吗？简述 go 内存管理机制。](#golanggo)
* [简述 Go 语言 GC(垃圾回收)的工作原理 ok](#GoGCok)
* [如何知道一个对象是分配在栈上还是堆上？](#-1)
* [json序列化与反序列化](#json)
* [defer的底层数据结构](#defer)
* [defer](#defer-1)
* [go 的 reflect 底层实现](#goreflect)
* [如何获取一个结构体的所有 tag？](#tag)
* [RPC 工作原理](#RPC)
* [rpc和http区别](#rpchttp)
* [grpc使用](#grpc)
* [grpc四种请求响应模式](#grpc-1)
...
* [生成器模式](#-1)
* [原型模式](#-1)
* [当 select 监控多个 chan 同时到达就绪态时，如何先执行某个任务？](#selectchan)
* [select 的实现原理](#select)
* [recover 的执行时机](#recover)
* [为什么有协程泄露(Goroutine Leak)？](#GoroutineLeak)
* [Go 可以限制运行时操作系统线程的数量吗？ 常见的 goroutine 操作函数有哪些？](#Gogoroutine)
* [如何控制协程数目](#-1)
* [defer 可以捕获 goroutine 的子 goroutine 吗？](#defergoroutinegoroutine)
* [go 竞态条件了解吗？](#go-1)
* [有三个函数，分别打印"cat", "fish","dog"要求每一个函数都用一个 goroutine，按照顺序打印 100 次。](#catfishdoggoroutine100)
* [两个协程交替打印 10 个字母和数字](#10)
* [启动 2 个 groutine 2 秒后取消，第一个协程 1 秒 执行完，第二个协程 3 秒执行完。](#2groutine213)
* [Go 什么时候发生阻塞？阻塞时，调度器会怎么做。](#Go-1)
* [指针的作用](#-1)
* [函数返回局部变量的指针是否安全？](#-1)
* [非接口的任意类型 T() 都能够调用 *T 的方法吗？反过来呢？](#TT)
* [lru](#lru)
* [Go变量](#Go-1)
* [数字类型](#-1)
* [strconv](#strconv)
...
* [= 和 := 的区别？ok](#:ok)
* [什么是 rune 类型](#rune)
* [Go 支持默认参数或可选参数吗？](#Go-1)
* [Go 语言 tag 的用处？](#Gotag)
* [结构体打印时，%v 和 %+v 的区别](#vv)
* [Go 语言中如何表示枚举值(enums)？](#Goenums)
* [空 struct{} 的用途 ok](#structok)
* [go 里面的 int 和 int32 是同一个概念吗？](#gointint32)
* [init() 函数是什么时候执行的？](#init)
* [uint 型变量值分别为 1，2，它们相减的结果是多少？](#uint12)
* [fallthrough](#fallthrough)
* [nil](#nil)
* [iota](#iota)
* [Go 中 Label 使用](#GoLabel)
* [数组](#-1)
* [切片](#-1)
	* [切片初始化加索引](#-1)
	* [切片拷贝三种方式](#-1)
	* [两个切片追加在一起](#-1)
	* [切片切取](#-1)
* [如何判断 2 个字符串切片（slice) 是相等的？](#2slice)
* [map](#map-1)
	* [通过 make 函数定义 map](#makemap)
	* [map 的排序](#map-1)
	* [map 中删除一个 key，它的内存会释放么？](#mapkey)
	* [遍历map为啥不是有序的？](#map-1)
	* [map三种并发安全的方式](#map-1)
* [Go 有异常类型吗？](#Go-1)
* [查看本地etcd服务](#etcd)
* [docker部署etcd服务](#dockeretcd)
	* [缓存实现高性能](#-1)
	* [缓存实现高并发](#-1)
	* [缓存雪崩](#-1)
	* [缓存击穿](#-1)
...
* [rocketmq](#rocketmq)
	...
	* [redis主从复制](#redis-1)
		* [主从复制核心原理](#-1)
	* [哨兵模式](#-1)
		* [成为候选者的条件：拿到半数的票数；大于等于quorum的值；](#quorum)
		* [故障转移过程](#-1)
		* [导致数据丢失的两种情况](#-1)
		* [哨兵的自动发现机制](#-1)
	* [redis过期删除策略](#redis-1)
	...
	* [docker-compose部署redis集群](#docker-composeredis)
...
### <a name='Gowindowsbuildlinux'></a>Go在windows环境下build生成linux程序
...
maptype、arraytype、chantype…其都是各个结构的公共描述，可以理解为一种外在的表现信息。
interfaetype 和 type 唯一确定了接口类型，而 hash 用于查询和类型判断。
fun 表示方法集。
eface 与 iface 基本一致，但是用_type 直接表示类型，这样的话就无法使用方法。 
type eface struct {
    _type *_type
    data  unsafe.Pointer 
}
...
## <a name='gorm'></a>gorm 里面如何批量插入某个字段 
...
## <a name='goroutine'></a>goroutine 什么情况会发生内存泄漏？如何避免。
...
正常模式 
所有 goroutine 按照 FIFO 的顺序进行锁获取，被唤醒的 goroutine 和新请求锁的goroutine 同时进行锁获取，
通常新请求锁的 goroutine 更容易获取锁(持续占有cpu)，被唤醒的 goroutine 则不容易获取到锁。
公平性：否。

饥饿模式 
所有尝试获取锁的 goroutine 进行等待排队，新请求锁的 goroutine 不会进行锁获取(禁用自旋)，
而是加入队列尾部等待获取锁。公平性：是。 
...
一些基本概念：
页Page：一块 8K 大小的内存空间。Go 向操作系统申请和释放内存都是以页为单位的。

span: 内存块，一个或多个连续的 page 组成一个 span。
如果把 page 比喻成工人，span 可看成是小队，工人被分成若干个队伍，不同的队伍干不同的活。

sizeclass: 空间规格，每个 span 都带有一个 sizeclass ，标记着该 span 中的 page 应该如何使用。
使用上面的比喻，就是 sizeclass 标志着 span 是一个什么样的队伍。
...
池进行管理，mheap 将一整块内存切割为不同的区域，并将一部分内存切割为合适的大小。 
...
## <a name='recover'></a>recover 的执行时机 
...
### <a name='rabbitmq'></a>rabbitmq的三种模式
1. 单机模式
2. 普通集群模式
...
#### <a name='quorum'></a>成为候选者的条件：拿到半数的票数；大于等于quorum的值；
...
### <a name='redis-1'></a>redis过期删除策略
redis会将key带上过期时间存储到一个过期字典中；
策略：惰性删除+定期删除
惰性删除策略的做法是，不主动删除过期键，每次从数据库访问 key 时，都检测 key 是否过期，如果过期则删除该 key。
定期删除策略的做法是，每隔一段时间「随机」从数据库中取出一定数量的 key 进行检查，并删除其中的过期key。

### <a name='key'></a>过期key处理
...
### <a name='docker-composeredis'></a>docker-compose部署redis集群
