...
### 两阶段提交
两阶段提交(Two-Phase Commit, 2PC)是一种常用于分布式事务处理的协议,它可以保证分布式事务的原子性。

两阶段提交的工作流程如下:

第一阶段 (准备阶段):
1. 协调者(Coordinator)向所有参与者(Participant)发送"准备"请求,询问是否可以提交事务。
2. 每个参与者执行事务操作,并将结果保存到事务日志中,但不提交。
3. 参与者向协调者反馈是否"准备好"提交。

第二阶段 (提交阶段):
1. 如果所有参与者都反馈"准备好",协调者向所有参与者发送"提交"请求。
2. 参与者根据事务日志,正式提交事务操作。
3. 参与者向协调者反馈提交结果。
4. 协调者收到所有参与者的提交结果后,完成事务提交。

如果在第一阶段有任何一个参与者反馈"未准备好",协调者会向所有参与者发送"中止"请求,参与者会中止事务并回滚。

两阶段提交协议的优点是:

1. 保证了分布式事务的原子性,要么全部成功,要么全部失败。
2. 通过引入协调者,简化了参与者之间的直接协调。
3. 支持异构系统的事务协调。

缺点是:
1. 协调者是单点故障,需要采取措施提高可靠性。
2. 两阶段过程较长,在网络或节点故障时容易造成阻塞。
3. 参与者需要支持事务日志等机制,增加了系统复杂度。

两阶段提交协议是分布式事务处理中广泛使用的一种经典解决方案,适用于对一致性要求较高的场景。


### 三阶段提交
三阶段提交(Three-Phase Commit, 3PC)是一种改进的分布式事务提交协议,相比于两阶段提交(2PC)具有更好的容错性。

三阶段提交的工作流程如下:

第一阶段 (CanCommit 阶段):
1. 协调者(Coordinator)向所有参与者(Participant)发送"CanCommit"请求,询问是否可以提交事务。
2. 每个参与者执行事务操作,并将结果保存到事务日志中,但不提交。
3. 参与者向协调者反馈是否"CanCommit"。

第二阶段 (PreCommit 阶段):
4. 如果所有参与者都反馈"CanCommit",协调者向所有参与者发送"PreCommit"请求。
5. 参与者根据事务日志,执行 PreCommit 操作,但仍不提交。
6. 参与者向协调者反馈 PreCommit 结果。

第三阶段 (DoCommit 阶段):
7. 如果所有参与者都反馈 PreCommit 成功,协调者向所有参与者发送"DoCommit"请求。
8. 参与者根据事务日志,正式提交事务操作。
9. 参与者向协调者反馈提交结果。
10. 协调者收到所有参与者的提交结果后,完成事务提交。

如果在任何一个阶段有参与者反馈失败,协调者会向所有参与者发送"Abort"请求,参与者会中止事务并回滚。

三阶段提交相比于两阶段提交的优点是:

1. 引入了 PreCommit 阶段,可以在提交前检查是否所有参与者都准备好了,从而避免了二阶段可能出现的阻塞问题。
2. 增加了容错性,即使在 DoCommit 阶段有节点失败,其他节点也可以完成事务提交。

缺点是:

1. 协调者仍是单点故障,需要采取措施提高可靠性。
2. 三阶段过程较长,增加了系统复杂度。
3. 参与者需要支持更复杂的事务日志等机制。

三阶段提交适用于对一致性和可用性要求都较高的分布式系统场景。


### Raft投票协议
Raft 是一种广泛使用的分布式一致性协议,它通过投票机制来选举领导者并维护复制日志的一致性。Raft 的投票协议主要包含以下几个步骤:

1. **选举领导者**:
   - 每个节点都可以成为候选人发起选举,以成为新的领导者。
   - 候选人首先会增加自己的任期号,然后向集群中的其他节点发送投票请求。
   - 其他节点会根据以下条件决定是否投票给该候选人:
     - 该候选人的任期号必须大于等于本节点已知的最大任期号。
     - 本节点的日志必须和候选人的日志一致或更新。
     - 本节点当前状态必须是跟随者(Follower)或候选人。
   - 如果候选人获得了集群中过半数节点的投票,就成为新的领导者。

2. **日志复制**:
   - 领导者负责接收客户端的写请求,并将其追加到自己的日志中。
   - 领导者会周期性地向跟随者发送附加日志条目的请求,要求跟随者将自己的日志更新到最新状态。
   - 跟随者会检查请求中的日志条目是否与自己的日志一致,如果一致则更新日志,否则拒绝更新。

3. **故障转移**:
   - 如果领导者崩溃或网络分区,跟随者会开始新一轮的选举,选出新的领导者。
   - 新的领导者会继续复制日志,确保集群达成一致。

Raft 的投票协议有以下几个特点:

1. 单一领导者:集群中任一时刻只有一个领导者,负责日志的写入和复制。
2. 安全性:新领导者的日志一定包含了之前所有已提交的日志条目。
3. 可用性:只要集群中过半数节点正常工作,就可以继续提供服务。

Raft 的投票协议简单易懂,同时具有较强的一致性和容错性,是分布式系统中广泛使用的一种经典方案。
